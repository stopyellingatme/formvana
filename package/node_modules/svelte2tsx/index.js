...
const createClassSetter = (name) => `\n    /**accessor*/\n    set ${name}(${name}) {}`;
const createClassSetters = (names) => {
    return Array.from(names).map(createClassSetter).join('');
};
function createRenderFunctionSetterStr(setters) {
...
function processInstanceScriptContent(str, script, events, implicitStoreValues, usesAccessors) {
    const htmlx = str.original;
...
function addComponentExport({ str, uses$$propsOr$$restProps, strictMode, strictEvents, isTsFile, getters, setters, fileName, componentDocumentation }) {
...
function createRenderFunction({ str, scriptTag, scriptDestination, slots, getters, setters, events, exportedNames, isTsFile, uses$$props, uses$$restProps, uses$$slots }) {
    const htmlx = str.original;
    let propsDecl = '';
    if (uses$$props) {
