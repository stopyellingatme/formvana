...
function visitImportDeclaration$4({ node, typescript, markAsImported }) {
...
function visitExportDeclaration$5({ node, typescript, markAsExported }) {
    var _a, _b;
...
function ensureNoDeclareModifierTransformer(options) {
    ...
    // Prepare some VisitorOptions
    const visitorOptions = Object.assign(Object.assign({}, options), { childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$a(Object.assign(Object.assign({}, visitorOptions), { node: nextNode })), context), continuation: (node) => visitNode$a(Object.assign(Object.assign({}, visitorOptions), { node })) });
...
function statementMerger({ markAsModuleIfNeeded }) {
    return options => {
        ...
        const visitorOptions = Object.assign(Object.assign({}, options), { preserveImportedModuleIfNeeded(module) {
            ...
            }, childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$9(Object.assign(Object.assign({}, visitorOptions), { node: nextNode })), context), continuation: (node) => visitNode$9(Object.assign(Object.assign({}, visitorOptions), { node })) });
        ...
        case typescript.SyntaxKind.ClassExpression: {
            return [
                ...
                preserveParents(compatFactory.createImportEqualsDeclaration.length === 4 ?
                    compatFactory.createImportEqualsDeclaration(undefined, undefined, compatFactory.createIdentifier(name), compatFactory.createQualifiedName(compatFactory.createIdentifier(moduleBinding), compatFactory.createIdentifier(propertyName))) : compatFactory.createImportEqualsDeclaration(undefined, undefined, false, compatFactory.createIdentifier(name), compatFactory.createQualifiedName(compatFactory.createIdentifier(moduleBinding), compatFactory.createIdentifier(propertyName))), { typescript })
            ];
        }
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function visitExportDeclaration$2(_options) {
    return undefined;
}
...
function visitImportEqualsDeclaration(options) {
    ...
    return node.name === nameContinuationResult
        ? node
        : preserveMeta(compatFactory.createImportEqualsDeclaration.length === 4 ? compatFactory.updateImportEqualsDeclaration(node, node.decorators, node.modifiers, nameContinuationResult, node.moduleReference) : compatFactory.updateImportEqualsDeclaration(node, node.decorators, node.modifiers, node.isTypeOnly, nameContinuationResult, node.moduleReference), node, options);
}
